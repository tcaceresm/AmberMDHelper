...
function CreateProtOnlyDirs() {
  ...
  log "  Protein-only directories under ${PROT_ONLY_BASE_DIR}"
}
...
log "CLI flags:"
log " --work_dir        : ${WDDIR}"
log " --prod_time       : ${PROD_TIME}"
log " --equi_time       : ${EQUI_TIME}"
log " --replicas        : ${REPLICAS}"
log " --prot_only       : ${PROT_ONLY_MD}"
log " --prot_lig        : ${PROT_LIG_MD}"
log " --prep_rec        : ${PREP_REC}"
log " --prep_lig        : ${PREP_LIG}"
log " --include_cof     : ${INCLUDE_COFACTOR}"
log " --prep_cof        : ${PREP_COFACTOR}"
log " --prep_topology   : ${PREP_TOPO}"
log " --prep_MD         : ${PREP_MD}"
...
log " --protocol        : ${PROTOCOL_ARGS[*]:-default}"
